...
samplemean = mean(puberty_sr$age)
puberty_sr$age_centered <- (puberty_sr$age - samplemean)

### SELF REPORT 

## log model for timing from PUBcomp 

b0 = 1
b1 = 5

# Nonlinear mixed effects model equation
model_equation <- function(b0, b1, alpha, lambda, age_centered) {
  b0 + (b1 - b0) * (1 / (1 + exp(-alpha * (age_centered - lambda))))
}

# Model
sr_model <- nlme::nlme(
  PUBcomp ~ model_equation(b0, b1, alpha, lambda, age_centered),
  fixed = b0 + b1 + alpha + lambda ~ 1,
  data = puberty_sr,
  random = list(alpha ~ 1, lambda ~ 1),
  start = c(b0 = 1, b1 = 5, alpha = 0.9, lambda = 13.5),
  control = nlme::nlmeControl(opt = "nlminb")
)

# Output of individual-level estimates
individual_estimates <- summary(sr_model)

# Output of individual-level predicted scores
predicted_scores <- predict(sr_model)

# Output of model fit statistics
fit_statistics <- summary(sr_model)$tTable

# Output of parameter estimates
parameter_estimates <- fixef(sr_model)
